...
void ktls_attach(int origfd, int opfd,  SSL *ssl) {
	printf("Entering in ktls_attach");
    ...
    unsigned char readIV[4];
    unsigned char writeIV[4];
    unsigned char readKey[16];
    unsigned char writeKey[16];

    RAND_bytes(writeKey, 16);
    RAND_bytes(writeIV, 4);
    RAND_bytes(readKey, 16);
    RAND_bytes(readIV, 4);
...
static int ssl_sock_init(struct connection *conn)
{
	...
	/*	int opfdcli = socket(AF_KTLS, SOCK_STREAM, 0);
    		if (opfdcli < 0) {
		...
		//ktls_attach(conn->t.sock.fd, opfdcli, conn->xprt_ctx);
	...
	retry_accept:
		/* Alloc a new SSL session ctx */
		SSL_CTX_set_ssl_version(objt_listener(conn->target)->bind_conf->default_ctx, TLSv1_2_method());
    		SSL_CTX_set_options(objt_listener(conn->target)->bind_conf->default_ctx, SSL_OP_NO_SSLv2);
    		SSL_CTX_set_cipher_list(objt_listener(conn->target)->bind_conf->default_ctx, "AES128-GCM-SHA256");
		...
		int opfdsrv = socket(AF_KTLS, SOCK_STREAM, 0);
                if (opfdsrv < 0) {
                        perror("AF_ALG: create socket failed\n");
                exit(EXIT_FAILURE);
                }
		...
		/* leave init state and start handshake */
		conn->flags |= CO_FL_SSL_WAIT_HS | CO_FL_WAIT_L6_CONN;
		//SSL_connect(conn->xprt_ctx);

		ktls_attach(conn->t.sock.fd, opfdsrv, conn->xprt_ctx);
