...
void ktls_attach(int sockfd, int opfd,  SSL *ssl) {
    struct sockaddr_ktls sa = { .sa_cipher = KTLS_CIPHER_AES_GCM_128,
            .sa_socket = sockfd, .sa_version = KTLS_VERSION_1_2};
...
static int ssl_sock_init(struct connection *conn)
{
	...
	retry_connect:
		...
		ktls_attach(conn->t.sock.d, opfdcli, conn->xprt_ctx);
	...
	retry_accept:
		...
		ktls_attach(conn->t.sock.d, opfdsrv, conn->xprt_ctx);
